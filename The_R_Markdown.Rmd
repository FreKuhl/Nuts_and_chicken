...
On the X-Axis the possible revenue distribution in € is displayed. The Y-Axis displays the probability density how likely one certain amount of revenue is likely. If the Curve for a scenario is wide, it means that the degree of uncertainty is high, the possible revenue range is large. The smaller the curve is, the more is the scenario defined and it is of a higher certainty that a certain revenue is reached.
...
Another option to further analyse the results is to use the `plot_chashflow()` option [@luedeling_decisionsupport_2021]. It plots the annual cashflow over the 30 years. It displays the median as well as the 50% and 90% confidence interval. 
